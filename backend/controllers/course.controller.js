...
export const getCourses = async (req, res) => {
  try {
    const courses = await Course.find({}).populate('creatorId', 'firstName lastName email');
    
    // Check which courses are already purchased and add default dates for courses without createdAt
    const coursesWithPurchaseStatus = await Promise.all(
      courses.map(async (course) => {
        const purchase = await Purchase.findOne({ courseId: course._id });
        
        // If course doesn't have createdAt, add a default date (1 day ago)
        if (!course.createdAt) {
          const defaultDate = new Date();
          defaultDate.setDate(defaultDate.getDate() - 1);
          course.createdAt = defaultDate;
        }
    ...
    res.status(201).json({ courses: coursesWithPurchaseStatus });
  } catch (error) {
...
export const buyCourses = async (req, res) => {
  ...
  try {
    ...
    // Check if this specific user has already purchased this course
    const existingUserPurchase = await Purchase.findOne({ userId, courseId });
    ...
    // Check if ANY user has already purchased this course (since it's a used item)
    const existingPurchase = await Purchase.findOne({ courseId });
    if (existingPurchase) {
      return res 
        .status(400)

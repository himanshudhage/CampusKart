...
function Items() {
  ...
  useEffect(() => {
    const fetchItems = async () => {
      try {
        const response = await axios.get("http://localhost:4002/api/v1/item/items", {
          withCredentials: true,
        });
        console.log("Fetched items:", response.data.items);
        console.log("First item structure:", response.data.items[0]);
        console.log("First item createdAt:", response.data.items[0]?.createdAt);
        console.log("First item updatedAt:", response.data.items[0]?.updatedAt);
        console.log("All item dates:", response.data.items.map(i => ({ 
          title: i.title, 
          createdAt: i.createdAt, 
          updatedAt: i.updatedAt 
        })));
        
        // Sort items: available items first (newest first), then sold items (newest first)
  ...
  const performSearch = (searchQuery) => {
    ...
    setTimeout(() => {
      if (searchQuery.trim() === "") {
        ...
        console.log("Cleared search, showing all items:", items.length);
      } else {
        ...
        const filtered = items.filter(item => {
          ...
          console.log(`Item "${title}" matches "${term}":`, matches);
          return matches;
        });
  ...
  useEffect(() => {
    const savedHistory = localStorage.getItem('itemSearchHistory');
    if (savedHistory) {
                      ...
                      <div className="flex items-center space-x-3">
                        <div className="w-8 h-8 bg-gradient-to-r from-orange-400 to-orange-600 rounded-full flex items-center justify-center">
                          <span className="text-white text-sm font-bold">
                            {(item.creatorId?.firstName || item.admin?.firstName || item.createdBy?.firstName || item.adminName || 'A').charAt(0).toUpperCase()}
                          </span>
                        </div>
                        <div>
                          <div className="text-orange-400 font-medium text-sm">
                            Posted by {item.creatorId?.firstName?.toLowerCase() || item.admin?.firstName?.toLowerCase() || item.createdBy?.firstName?.toLowerCase() || item.adminName?.toLowerCase() || 'admin'}
                          </div>
          ...
          <div className="text-center py-16">
            <div className="bg-white/5 backdrop-blur-lg rounded-2xl p-12 border border-white/10 shadow-2xl max-w-md mx-auto">
              ...
              <p className="text-gray-300 mb-8 font-inter leading-relaxed">
                {searchTerm 
                  ? `No items match your search for "${searchTerm}". Try different keywords or browse all items.`
                  : "We're working on adding amazing items for you. Check back soon for new deals!"
                }
              </p>

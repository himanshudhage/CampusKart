...
function UpdateItem() {
  ...
  useEffect(() => {
    const fetchItemData = async () => {
      try {
        const { data } = await axios.get(`http://localhost:4002/api/v1/item/${id}`, {
  ...
  const handleUpdateItem = async (e) => {
    ...
    if (image && typeof image === "object") {
      ...
      formData.append("image", JSON.stringify({ url: image }));
    }
    ...
    try {
      const response = await axios.put(
        `http://localhost:4002/api/v1/item/update/${id}`,
        formData,
        {
          headers: {
      ...
      toast.error(error.response?.data?.errors || "Failed to update item");
    }
  };
    ...
    <div className="flex h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 text-white">
      ...
      <div className={`flex-1 p-8 overflow-auto transition-all duration-300 ${isSidebarOpen ? "ml-72" : "ml-0"} md:ml-72`}>
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-8 gap-4">
          <div>
            <h2 className="text-4xl font-poppins font-bold text-orange-400 drop-shadow-md tracking-tight mb-2">Update Item</h2>
            <p className="text-gray-400 font-inter">Edit your marketplace listing</p>
          </div>
